...
class Term:
    ...
    def __repr__(self):  # eval(myTerm)
        ...
        '''
        return f"Term({self.coef}, {self.exp})"
...
class LinkedPolynomial:
    ...
    def createFromNumbers(self, numList):
        ...
        for i in range(len(numList)):
            if numList[i-1][1] != numList[i][1] or i==0:
                pList.append(Term(numList[i][0], numList[i][1]))
            else:
                pList[-1] = Term(numList[i][0] + pList[-1].coef , numList[i][1])
        self.polyList = list(filter(lambda x: x.coef != 0, pList))
    ...
    def __neg__(self):  # negPoly = -myPoly
        ...
        '''
        out = LinkedPolynomial(list(map(lambda x: -x, self.polyList)))
        return out
    ...
    def __mul__(self, otherPoly):  # mulPoly = myPoly * otherPoly
        ...
        '''
        out = LinkedPolynomial()
        l = []
        for i in self.polyList:
            for j in otherPoly.polyList:
                l.append(((i.coef*j.coef), (i.exp+j.exp)))
        out.createFromNumbers(l)
        return out